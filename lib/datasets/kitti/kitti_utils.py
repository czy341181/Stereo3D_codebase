...
class Calibration(object):
    ...
    def lidar_pseudo_to_rect(self, pts_lidar):
        pts_lidar_hom = self.cart_to_hom(pts_lidar)
        T = np.array([[0, 0, 1],
                      [-1, 0, 0],
                      [0, -1, 0],
...
def boxes3d_kitti_camera_to_pselidar(boxes3d_camera, calib):
    """
    Args:
        boxes3d_camera: (N, 7) [x, y, z, l, h, w, r] in rect camera coords
        calib:
    ...
    xyz_camera = boxes3d_camera[:, 0:3]
    l, h, w, r = boxes3d_camera[:, 3:4], boxes3d_camera[:, 4:5], boxes3d_camera[:, 5:6], boxes3d_camera[:, 6:7]
    xyz_lidar = calib.rect_to_lidar_pseudo(xyz_camera)
    xyz_lidar[:, 2] += h[:, 0] / 2
    return np.concatenate([xyz_lidar, l, w, h, -(r + np.pi / 2)], axis=-1)


def random_flip_horizontal_2d(image_l, image_r,  gt_bbox3d, calib):
    """
    ...
    if enable:
        # Flip images
        aug_image_r = np.fliplr(image_l)
        aug_image_l = np.fliplr(image_r)
    ...
    return aug_image_l, aug_image_r, aug_gt_bbox3d, aug_calib

def random_flip_horizontal_3d(image_l, image_r, gt_bbox3d, calib):
    enable = np.random.choice([False, True], replace=False, p=[0.5, 0.5])
    #enable = True
    if enable:
        # Flip images
        ...
        aug_gt_bbox3d = np.concatenate([-gt_bbox3d[:, 0:1], gt_bbox3d[:, 1:6], alpha], axis=1)
...
def random_flip(image_l, image_r, left_bbox, right_bbox, left_project_center, right_project_center, calib):
    ...
    if enable:
        # Flip images
        W = image_l.size[0]
        aug_image_r = image_l.transpose(Image.FLIP_LEFT_RIGHT)
        aug_image_l = image_r.transpose(Image.FLIP_LEFT_RIGHT)
        #aug_depth_map = np.fliplr(depth_map)
        aug_calib = copy.copy(calib)

        aug_right_bbox = copy.copy(left_bbox)
        aug_left_bbox = copy.copy(right_bbox)

        aug_left_bbox[:, 0] = W - right_bbox[:, 2]
        aug_left_bbox[:, 2] = W - right_bbox[:, 0]
...
def boxes3d_kitti_camera_to_imageboxes(boxes3d, calib, image_shape=None, fix_neg_z_bug=False):
    """
...
def boxes3d_pselidar_to_kitti_camera(boxes3d_lidar, calib=None):
    """
    ...
    xyz_cam = calib.lidar_pseudo_to_rect(xyz_lidar)
    # xyz_cam[:, 1] += h.reshape(-1) / 2
    r = -r - np.pi / 2
    return np.concatenate([xyz_cam, l, h, w, r], axis=-1)

def boxes3d_lidar_to_kitti_camera(boxes3d_lidar, calib=None):
    """
    :param boxes3d_lidar: (N, 7) [x, y, z, dx, dy, dz, heading], (x, y, z) is the box center
    :param calib:
    :return:
