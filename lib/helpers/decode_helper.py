...
def decode_detections(dets, frame_id, bbox_downsample_ratio, calibs, threshold):
    '''
    ...
    for i in range(dets.shape[0]):  # batch
        ...
        for j in range(dets.shape[1]):  # max_dets
            ...
            # 2d bboxs decoding
            x = dets[i, j, 2] * bbox_downsample_ratio[i][0]
            y = dets[i, j, 3] * bbox_downsample_ratio[i][1]
            w = dets[i, j, 4] * bbox_downsample_ratio[i][0]
            h = dets[i, j, 5] * bbox_downsample_ratio[i][1]
            ...
            # positions decoding
            #x3d = dets[i, j, 34] * bbox_downsample_ratio[i][0]
            #y3d = dets[i, j, 35] * bbox_downsample_ratio[i][1]
            ...
            preds.append([cls_id] + bbox + [score])
            #preds.append([cls_id, alpha] + bbox + dimensions.tolist() + locations.tolist() + [ry, score])
        results[frame_id[i]] = preds
    return results


def extract_dets_from_stereo_outputs(outputs, K=50):
    # get src outputs
    heatmap = outputs['heatmap']

    size_2d_left = outputs['size_2d_left']
    offset_2d_left = outputs['offset_2d_left']

    width_right = outputs['width_right']
    offset_2d_right = outputs['offset_2d_right']
    ...
    offset_2d_left = _transpose_and_gather_feat(offset_2d_left, inds)
    offset_2d_left = offset_2d_left.view(batch, K, 2)
    xs2d_l = xs.view(batch, K, 1) + offset_2d_left[:, :, 0:1]
    ys2d_l = ys.view(batch, K, 1) + offset_2d_left[:, :, 1:2]

    offset_2d_right = _transpose_and_gather_feat(offset_2d_right, inds)
    offset_2d_right = offset_2d_right.view(batch, K, 2)
    xs2d_r = xs.view(batch, K, 1) + offset_2d_right[:, :, 0:1]
    ys2d_r = ys.view(batch, K, 1) + offset_2d_right[:, :, 1:2]
    ...
    # check shape
    xs2d_l = xs2d_l.view(batch, K, 1)
    ys2d_l = ys2d_l.view(batch, K, 1)
    xs2d_r = xs2d_r.view(batch, K, 1)
    ys2d_r = ys2d_r.view(batch, K, 1)

    size_2d_left = _transpose_and_gather_feat(size_2d_left, inds)
    size_2d_left = size_2d_left.view(batch, K, 2)
    width_right = _transpose_and_gather_feat(width_right, inds)
    width_right = width_right.view(batch, K, 1)
    ...
    return detections_l, detections_r
