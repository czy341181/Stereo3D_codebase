...
class IDAUp(nn.Module):
    ...
    def __init__(self, o, channels, up_f, dcn):
...
class DLAUp(nn.Module):
    def __init__(self, startp, channels, scales, in_channels=None, dcn=None):
        ...
        for i in range(len(channels) - 1):
            ...
            setattr(self, 'ida_{}'.format(i),
                    IDAUp(channels[j], in_channels[j:], scales[j:]//scales[j], dcn=dcn))
            scales[j + 1:] = scales[j]
    ...
    def forward(self, layers):
        ...
        #len(layer): 6  startp: 3
        for i in range(len(layers) - self.startp - 1): #2
...
class DLASeg(nn.Module):
    def __init__(self, base_name, pretrained, down_ratio, final_kernel,
                 last_level, head_conv, dcn, out_channel=0):
        ...
        self.dla_up = DLAUp(self.first_level, channels[self.first_level:], scales, dcn=dcn)
        if out_channel == 0:
            out_channel = channels[self.first_level]
            self.out_channels = out_channel

        self.ida_up = IDAUp(out_channel, channels[self.first_level:self.last_level],
                            [2 ** i for i in range(self.last_level - self.first_level)], dcn)
